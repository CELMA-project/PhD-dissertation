...
The modes will either grow or be damped in the linear phase, depending on the simultion parameters.
...
However, our system is not purely linear, mode coupling will start to become important once the perturbations have grown sufficiently big.
...
\footnote{Altough the time trace of \cref{fig:fourierStable} shows a more complex behavior than just growing or just decayin, we conclude that the mode is stable against perturbations, as they remain small for all times, and that every "attempt" of growth is followed by a raprid decay.
    The growth is obseved if the mode stops rotating, when it starts rotating again, a decay of the mode is observed.
