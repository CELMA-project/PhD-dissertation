...
In order to ensure that the system has reached a steady state, the simulations are therefore executed until $4000t\om_{ci}$ is reached.

Secondly, the simulation is expanded to $n_\theta = 256$, and executed for additional $50t\om_{ci}$.
...
This has been found to give a good speed-up (as compared to use one node) with a good trade off between the ratio of time spent on arithmetics comparedto communication (see \cref{chap:performance}).
