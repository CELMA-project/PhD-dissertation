In this chapter we will make a comparison between the simulation using the full set of CELMA equations (\textbf{C}ELMA-\textbf{F}ull, or simply CF), with the simutations using the Boussinesq approximation (\textbf{C}ELMA-\textbf{B}oussinesq, or simply CB).
...
This $n_0$ then got normalized out, meaning that the $n$-dependency in this term dissapeared.
As a result, we ended up with a source term in the CB model not present in the CF model, since this term got cancelled with the source term from the density equation which we "smuggled" inside the $\d_t^E$ operator in the CF case.
...
%
.
The radial density profile stays roughly the same for the CF and CB case, which will be dicussed in further detail below.
...
On the other hand, the parallel parallel current profile has changed, as seen in \cref{fig:compareBoussProfPar}.
...
The intermediate pahse between the linear phase and the saturated phase is relatively short.
...
The final state for $B_0=0.06\T$ seems to be undeterminate.
...
The reuslt is given in \cref{fig:GRBM,fig:GRBB}.
%
\begin{figure}[htb]
    \centering
...
Secondly, the time trace of the density fluctation appears to be more ordered, in the sense that they appear more periodic than tose observed in \cref{fig:combinedPlots008} for the CF case.
