...
For a mathematical illustration of this, see for example \cite{Garcia2001a}.
...
\end{align}
%
which has the following solution for the maximum growth rate
%
\begin{align*}
...
\end{align*}
%
We not that the maximum growth (the highest positive $\Im(\om)$) in \cref{eq:grIm} is obtained where $\om^{*}$ has a maximum, which corresponds to the position of the minimum in $\partial_x n/n$.
...
\end{figure}
%
From \cref{fig:grAnalyticBModeNr} we see that the growth rates of all modes are monotonically increasing with increasing $B_0$, apart from $m_\theta=1$.
We can also observe that the angular frequency increases with increasing $B_0$.
This migth come as a surprise, as simpler models for the drift wave predicts that $\Re(\om)\propto \om^* \propto 1/B$.
...
However, as shown in \cref{tb:typicalLinValsB,tb:typicalLinVals} this is not true for our case, where $\Re(\om)\propto B$ is observed.
%
\begin{table}[h!]
...
FIXME:
FIXME: Corrected for uE in foo bar
...
\begin{align*}
    \Re[\om_n(t)] =
    \frac{
        \atan2\L(\Im[\text{FT}(n[t_1])], \Re[\text{FT}(n[t_1])]\R) -
        \atan2\L(\Im[\text{FT}(n[t_0])], \Re[\text{FT}(n[t_0])]\R)
    }{\Delta t}
\end{align*}
%
...
A summary of how the growth rates and rotations scales with the magnetic field $B$ is given in \cref{fig:grB,fig:grBModeNr}.
%
...
LEAVE THIS IN OBTAINING DISPERSION

As indicated in \cref{fig:grB,fig:grBModeNr}, which poloidal mode dominates is dependent on $B$.
