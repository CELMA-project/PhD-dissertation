...
Here, however, we simplify the expression (on the cost of accuracy), and say that we assume that the viscosities are constants (note that we already did such an approximation when calculating the resistivity in \cref{sec:collDrift}).
We then get
%
\begin{align*}
    \div\te{\pi}_\a
...
\begin{figure}[t!]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \caption{The normalized electron viscosities as a function of $n$}
    \end{subfigure}
    \\
    \hfill
    \begin{subfigure}[t]{0.45\textwidth}
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \caption{The normalized electron viscosities as a function of $T_e$}
    \end{subfigure}
...
\end{align*}
%
We will use that, to first order, only the $\ve{E}\times\ve{B}$ drift advects particles perpendicularly.
...
\end{align*}
%
We note that we are operating with a left-handed coordinate system.
...
\end{align*}
%
We found in \cref{app:DO} that the viscosity played a minor role in the perpendicular velocities (assuming that $\mu \e^2$ is small in the ion equation).
