...
We can now solve \cref{eq:doStart} for $\ve{u}_{\a,\perp}$ by using perturbation theory.
...
%
since $\e\ll1$.
The basic idea is to split equation \cref{eq:doStart} into one equation for each order (i.e. equation $n$ would only contain of $\mathcal{O}(\e^n)$ terms).
The solution to the $\mathcal{O}(\e^0)$ will then be an approximate solution to the system.
The first order correction would be given by the solution to the $\mathcal{O}(\e^1)$ equation, the second order correction by the solution to the $\mathcal{O}(\e^2)$ equation and so on.
...
If we truncate after the first order, we get that
%
\begin{align}
...
\end{align}
%
If we insert \cref{eq:uOrder} into \cref{eq:doStart}, we get
%
\begin{align*}
 \overbrace{
