...
\end{align*}
%
We will now assume that the plasma consists of only one type of ions (i.e. all the ions have the same ionization, and are in the same excitation state).
...
This gives an analytical expression for $\ve{R}_{\b\to\a}$ (see \cref{app:collisions}) and the components of $\te{\pi}_\a$ (see \cref{app:piTensor}), and the set of equations (including the energy equations) are known as the Braginskii equations.
