...
We will now work through the zeroth moment term by term by applying $\expt{n_\a \;\cdot\;}_f=n_\a\expt{\;\cdot\;}_f$ on equation (\ref{eq:fp}).
We have that
%
\begin{align*}
...
We will now work through the first moment term by term by applying $\expt{m_\a n_\a \ve{v} \;\cdot\;}_f=m_\a n_\a\expt{ \ve{v} \;\cdot\;}_f$ on equation (\ref{eq:fp}).
We have that
%
\begin{align*}
...
The second term of equation (\ref{eq:mom_crude}) can now be written as
%
\begin{align*}
    \div\te{\Pi}_\a
      =&
...
\end{align*}
%
If we now introduce $\d_{t,\a} = \partial_t + \ve{u}_{\a}\cdot\nabla$ and insert this into equation (\ref{kin:mom_wo_full_deriv}) we get
%
\begin{align*}
...
%
By neglecting collision with neutral species, we have found that the two first moments of the Fokker-Planck equations ((\ref{kin:cont}) and (\ref{kin:mom})) can be written like
%
\begin{empheq}[box=\tcbhighmath]{align}
...
One often used method to close the set of equations, is the one suggested by Braginskii in his paper from $1965$ \cite{Braginskii1965}, which builds on the kinetic closure of gases derived by Chapman \cite{Chapman1916} and Enskog \cite{Enskog1917, Enskog1922}.
Although the temperature dynamics have been shown to play a major role in turbulence dynamics (see for example
% NOTE: Consider to add better references
\cite{Olsen2016}
