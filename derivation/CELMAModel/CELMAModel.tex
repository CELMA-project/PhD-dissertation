...
%
After the derivations in \cref{chap:slowB}, we are in the poisition to find the vorticity with our current approximations.
...
However, as we are deriving the voriticity equation from the conservation of charge (see \cref{eq:current_drifts}), we already have a term on the form $\div(n\ve{u}_{i,p})$.
...
\end{align*}
%
Note that $\frac{1}{\om_{ci}} \div\L( \frac{\grad_\perp \phi}{B} S_{i,n} \R)$ arising from the time derivative of $n_i$ in the polarization term of the current conservation equation has cancelled with the same term with opposite sign arising from the source term of the current conservation equation.
