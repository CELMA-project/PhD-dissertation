...
The variation of the $nu_{i,\|}$ field is smaller than the variation in the $u_{i,\|}$ as the sheath boundary condition (to be treated in \label{sec:BCs}) will will accelerate the ions to the ion acoustic speed, and thus create a lower density around the sheath as compared with the rest of the paralell domain.
A smaller variation in the domain has the advantage that the difference in magnitude of the largest and smallest eigenvalues decreases, which is benifitial for the stiffness of the system \cite{Leveque2007book}.

To derive the current equation and parallel density momentum equation, we will start from \cref{eq:mom_crude}.
We have that
%
\begin{align*}
