...
In the parallel dirction there will be formation of plasma sheaths between the plasma and the material which is due to the difference in mobility between the ion species and the electron species \cite{Stangeby2000book}.
...
Firstly, the plasma does not usually extent all the way to the chasis of the machine, but is gradually terminated as the temperature drops and neutral dynamics takes over towards the edge.
Secondly, assuming that the plasma could extend all the way out to the chasis, the non-slip condition encountered in neutral fluid dynamics \cite{Kundu2010book} is not necessarily valid as the plasma have no way not adhere to the surface as a neutral fluid would do.

The boundaries for the cylindrical plasma consist of four parts.
Parallely, we have the SE at one end.
...
To be more specific the velocities changes sign at this point, and hece must be exactly zero there.
...
In this thesis, we set $\phi_0=\Lambda T_e/e$, so that $\phi=0$ for ampipolar flow %
%
\footnote{We do this as we have observed that the inversion algorithm for $\phi$ mentionded in \cref{app:lapInv} is more numerically stabilie}%
%
.
Normalization gives
%
...
In the current implementation $\L. \partial_\| \Om \R|_{L_z} = 0$ has been choosen.
Note that this may not be a particularily good boundary condition as it physically means that there can be no parallel shear in the perpendicular spinning at the SE.
...
are reaching the domian boundary.
...
%
The parallel velocities are not contrained by any forces at the radial edge, and can thus take any value.
...
Thus, if we assume that the chassis is an ideal conductor, the potential there should be at the floating potential if we exlude finite Larmor radius (FLR) effects.
...
% FIXME: There is a see blackboard photo: BC-rho-on-vort, which could say what this should be
%
As stated in the introduction of \cref{sec:BCs}, there exsists no analouge to the no-slip condition in plasmas.
